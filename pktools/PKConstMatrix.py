...
def Dirac(label=-1):
    temp = PKMatrix([])
    if label not in [0, 1, 2, 3, 5]:
        print("ERROR: Invalid option for 'Dirac', number must be 0, 1, 2, 3 or 5")
    elif label is 0:
...
def GellMan(label=1):
    ...
    if label not in [i for i in range(1,9)]:
        print("ERROR: Invalid option for 'Gell-Mann', number must be 1, 2, 3, 4, 5, 6, 7, or 8")
    elif label is 1:
